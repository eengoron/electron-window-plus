 ...
 * @param {string} url
 *
 * Set the URL property of a given window to a specified URL. Useful when URLs are not loaded from the
 * windowPlus loadURL method, but you want the correct URL to be restored. 
 */
windowPlus.setURL = function (win, url) {
  let info = _getWinInfo(win.id);
  if ( info ) {
    info.url = url;
  }
